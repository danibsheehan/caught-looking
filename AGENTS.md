...
When editing **README.md**, global tokens in **`frontend/src/styles/_base.scss`**, or stack sources of truth (**`frontend/package.json`**, **`backend/go.mod`**, **`.github/workflows/ci.yml`**), keep docs aligned with the repo:
...
- **CI**: if check commands, jobs, or coverage-comment behavior change, update the README (including the **CI & quality gates** summary and Makefile tables where relevant), **`.github/workflows/ci.yml`**, and the PR template in the same spirit.
