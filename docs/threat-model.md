...
6. **API host in CSP** — `connect-src` allowlists the exact Cloud Run hostname(s), not a `*.run.app` wildcard (which would permit connecting to any Cloud Run service on the internet). Production and PR previews currently resolve `VITE_API_BASE` to two different valid aliases for the same Cloud Run service (see `frontend/public/_headers`'s comment); both are listed. If either alias, the region, or a custom domain changes, update `frontend/public/_headers` in the same change as `VITE_API_BASE`.
