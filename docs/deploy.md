...
| Same-repo PR, after preview deploys | [`.github/workflows/pages-preview.yml`](../.github/workflows/pages-preview.yml) (`lighthouse` job) | Runs Lighthouse CI against the deployed preview URL (real Pages + Cloud Run, not a standalone build); posts a sticky PR comment with the report link. Thresholds in [`.lighthouserc.json`](../.lighthouserc.json) are **warn**-level — informational only, does not block merge |
