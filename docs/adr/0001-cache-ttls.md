...
- **Follow-ups (not required):** optional shared L2 cache if `max-instances` grows; HTTP `Cache-Control` aligned with settle state for browsers — separate from this in-process policy.
