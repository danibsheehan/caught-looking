import { adjustChartColorForSurface, CHART_INK_MIN_CONTRAST } from './chartColorContrast';
...
export function neonThemeSeriesFills(count: number, surfaceHex: string): string[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    out.push(adjustChartColorForSurface(seed, surfaceHex, CHART_INK_MIN_CONTRAST));
  }
  return out;
}
