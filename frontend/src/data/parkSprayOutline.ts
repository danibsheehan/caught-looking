...
export function buildSprayFenceGeometry(dims: FieldDimensionsFt): SprayFenceGeometry {
  ...
  const first: [number, number] = [home[0] + BASE_PATH_FT * uRf[0], home[1] + BASE_PATH_FT * uRf[1]]
  const third: [number, number] = [home[0] + BASE_PATH_FT * uLf[0], home[1] + BASE_PATH_FT * uLf[1]]
  const second: [number, number] = [
    home[0] + BASE_PATH_FT * SQRT2 * uCf[0],
    home[1] + BASE_PATH_FT * SQRT2 * uCf[1],
  ]

  const { grass: infieldGrass, dirtOuter: infieldDirtOuter, moundCenter } = buildInfieldArt(
    home,
    first,
    second,
    third,
