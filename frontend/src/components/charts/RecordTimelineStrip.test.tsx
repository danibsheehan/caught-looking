...
describe('RecordTimelineStrip', () => {
  it('shows selection prompt for null/undefined values', () => {
    ...
    expect(screen.getByText(/Select a team and season to see the result strip/i)).toBeInTheDocument()
  })
  ...
  it('renders ordered streak transitions and tie legend branch', async () => {
    api.fetchRecordTimeline.mockResolvedValueOnce(
      record([
        { gameIndex: 1, officialDate: '2026-04-01', result: 'W', wins: 1, losses: 0, pct: 1 },
        { gameIndex: 2, officialDate: '2026-04-02', result: 'W', wins: 2, losses: 0, pct: 1 },
        { gameIndex: 3, officialDate: '2026-04-03', result: 'L', wins: 2, losses: 1, pct: 0.667 },
        { gameIndex: 4, officialDate: '2026-04-04', result: 'T', wins: 2, losses: 1, pct: 0.667 },
      ]),
    )
    render(<RecordTimelineStrip teamId={121} season={2026} />)
  ...
  it('does not show tie legend when no ties are present', async () => {
    api.fetchRecordTimeline.mockResolvedValueOnce(
      record([
        { gameIndex: 1, officialDate: '2026-04-01', result: 'W', wins: 1, losses: 0, pct: 1 },
        { gameIndex: 2, officialDate: '2026-04-02', result: 'L', wins: 1, losses: 1, pct: 0.5 },
      ]),
    )
    render(<RecordTimelineStrip teamId={121} season={2026} />)
