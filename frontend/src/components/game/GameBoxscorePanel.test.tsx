...
describe('GameBoxscorePanel', () => {
  ...
  it('renders team totals, pitching, and batting without runs-by-inning when gamePk is omitted', () => {
    ...
    expect(screen.queryByRole('heading', { name: 'Runs by inning', level: 2 })).not.toBeInTheDocument()
    ...
    expect(screen.queryByRole('heading', { name: 'Pitch location', level: 2 })).not.toBeInTheDocument()
  })
  ...
  it('shows pitch-location loading and error states', () => {
    const { rerender } = render(
      <GameBoxscorePanel
        data={box}
        pitchLocation={{ loading: true, error: null, data: null }}
      />,
    )
    ...
    rerender(
      <GameBoxscorePanel
        data={box}
        pitchLocation={{ loading: false, error: new Error('statcast unavailable'), data: null }}
      />,
    )
