...
<!-- perf-metrics:start -->
Warm cache-hit p95: **4.8ms** (target < 100ms) · cold-miss latency stays flat as concurrency scales to 500 clients, proving singleflight coalescing rather than serialized upstream calls — measured 2026-09-07. Cold numbers reflect a fixture-only injected test delay, not real-world timing — see [the full sweep and methodology](docs/perf-results.md).
<!-- perf-metrics:end -->
...
`make install` also sets up a **Husky** pre-commit hook (`.husky/pre-commit`) that runs
**lint-staged**, formatting staged `frontend/` files with Prettier before each commit — the
same check `npm run format:check` enforces in CI.
